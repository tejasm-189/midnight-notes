            ...
            div { style: "display: flex; height: 100vh; background: {c.bg_primary};",
                ...
                nav { style: "width: 256px; min-width: 256px; border-right: 1px solid {c.border}; background: {c.bg_surface}; display: flex; flex-direction: column; padding: 16px 0; font-family: 'JetBrains Mono', monospace; font-size: 12px;",
                    ...
                    button { style: "margin: 0 16px 16px; display: flex; align-items: center; justify-content: center; gap: 6px; padding: 8px; background: {c.accent}; color: {c.bg_primary}; border: none; border-radius: 4px; font-size: 11px; cursor: pointer;",
                        onclick: move |_| { show_settings.set(false); show_calendar.set(false); if let Some(ref d) = db_new { if let Ok(n) = NoteService::new(d).create("", "") { title.set(n.title.clone()); content.set(n.content.clone()); selected_id.set(Some(n.id.clone())); notes.write().insert(0, n); note_tags.set(vec![]); all_tags.set(vec![]); } } },
                        span { class: "material-symbols-outlined", style: "font-size: 16px;", "add" } "New Note"
                    }
                    div { style: "flex: 1; overflow-y: auto;",
                        SidebarItem { icon: "description", label: "All Notes", active: matches!(view.read().clone(), View::AllNotes) && !*show_settings.read() && !*show_calendar.read(), onclick: move |_| { query.set(String::new()); view.set(View::AllNotes); show_settings.set(false); show_calendar.set(false); refresh_notes(&db_side_all, &View::AllNotes, &notes, ""); } }
                        SidebarItem { icon: "auto_awesome", label: "Smart Views", active: matches!(view.read().clone(), View::SmartViews) && !*show_settings.read() && !*show_calendar.read(), onclick: move |_| { view.set(View::SmartViews); show_settings.set(false); show_calendar.set(false); } }
                        SidebarItem { icon: "calendar_month", label: "Calendar", active: *show_calendar.read() && !*show_settings.read(), onclick: move |_| { let c = *show_calendar.read(); show_calendar.set(!c); show_settings.set(false); } }
                        SidebarItem { icon: "today", label: "Daily Note", active: false, onclick: move |_| {
                            show_settings.set(false); show_calendar.set(false);
                            view.set(View::AllNotes); query.set(String::new());
                        ...
                        SidebarItem { icon: "archive", label: "Archived", active: matches!(view.read().clone(), View::Archived) && !*show_settings.read() && !*show_calendar.read(), onclick: move |_| { view.set(View::Archived); show_settings.set(false); show_calendar.set(false); refresh_notes(&db_side_arch, &View::Archived, &notes, ""); } }
                        SidebarItem { icon: "delete", label: "Trash", active: matches!(view.read().clone(), View::Trash) && !*show_settings.read() && !*show_calendar.read(), onclick: move |_| { view.set(View::Trash); show_settings.set(false); show_calendar.set(false); refresh_notes(&db_side_trash, &View::Trash, &notes, ""); } }
                        ...
                        // Tag tree
                        div { style: "border-top: 1px solid {c.border}; margin: 8px 16px;" }
                        crate::ui::sidebar::tag_tree::TagTree { key: "{all_tags.read().len()}", db: db_tag5.clone(), on_search_tag: move |tag_name| { view.set(View::AllNotes); query.set(format!("tag:{}", tag_name)); } }
                    }

                    // Bottom: export/import + lock
                    div { style: "border-top: 1px solid {c.border}; padding: 8px;",
                        SidebarItem { icon: "file_download", label: "Export", active: false, onclick: move |_| { show_settings.set(false); show_calendar.set(false); if let Some(ref d) = db_export { let ids: Vec<String> = notes.read().iter().map(|n| n.id.clone()).collect(); let ids_ref: Vec<&str> = ids.iter().map(|s| s.as_str()).collect(); let path = std::path::Path::new("midnight-export.zip"); let _ = ExportService::new(d).export_notes(&ids_ref, path, "export-pass"); } } }
                        SidebarItem { icon: "file_upload", label: "Import", active: false, onclick: move |_| { show_settings.set(false); show_calendar.set(false); let path = std::path::Path::new("midnight-export.zip"); if path.exists() { if let Some(ref d) = db_import { let _ = ExportService::new(d).import_notes(path, "export-pass"); let mut n = notes; n.write().clone_from(&vec![]); refresh_notes(&db_import, &View::AllNotes, &notes, ""); } } } }
                        div { style: "height: 1px; background: {c.border}; margin: 4px 0;" }
                        SidebarItem { icon: "settings", label: "Settings", active: *show_settings.read() && !*show_calendar.read(), onclick: move |_| { let s = *show_settings.read(); show_settings.set(!s); show_calendar.set(false); } }
                        SidebarItem { icon: "lock", label: "Lock Vault", active: false, onclick: move |_| { show_settings.set(false); show_calendar.set(false); on_lock.call(()); } }
                    }
                }
